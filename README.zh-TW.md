...
<p align="center">
  <strong>AI Tutor 幫你讀懂真正想讀的原文。</strong><br />
  VocabReader 把懂上下文的 AI Tutor 放在你的 EPUB 原文旁。<br />
  先讀懂當下的段落，再把你選中的內容帶進學習卡、間隔複習、造句與跟讀；每讀一本，就更能讀懂下一本。
</p>
...
![VocabReader 在 EPUB 原文旁顯示 AI Tutor](docs/images/reading-with-ai.png)

## 不離開書本，直接請 AI Tutor 講解

真正想讀的書，往往比目前程度再難一點。你可以掌握大意，卻不斷被陌生單字、片語與句型打斷。AI Tutor 會根據你選定的閱讀上下文，說明這裡的字詞、句子、文法或語氣，不必離開書本，也不必反覆複製貼上原文。

一般閱讀器可以顯示 EPUB，聊天工具可以解釋難句，卡片 App 可以安排複習。VocabReader 把原文、懂上下文的 AI Tutor、學習卡與間隔練習接在同一個閱讀流程裡。

VocabReader 把這段流程接在一起：

**匯入自己的 EPUB → 閱讀並標記阻礙理解的地方 → 在上下文中讀懂 → 保存值得學的內容 → 透過間隔複習持續鞏固 → 在造句與跟讀中運用**

它不是用 AI 取代閱讀，也不是替使用者自動翻譯整本書。設計重點是保留自己閱讀與思考的過程，只在需要時取得針對性的幫助，再把真正值得學的內容帶進後續練習。

### 核心特色

- **請懂這段原文的 AI Tutor 講解**：根據你選定的閱讀上下文說明，不必反覆複製原文。
- **從真實閱讀累積詞彙**：把單字和片語連同書中真正遇到的語義與例句保存到生詞庫。
- **持續複習重要內容**：讀過這一頁後，繼續透過間隔複習、閱讀測驗、造句與跟讀加深理解與記憶。
- **資料保存在本機**：書籍、閱讀進度、標記、生詞與複習紀錄都保存在目前裝置，也可以手動備份。

## 主要功能

### 1. 不離開書本，直接請 AI Tutor 講解
...
- 已具備足以掌握原文大意的基礎，但閱讀時仍會遇到稍微超出程度的語言。
...
VocabReader 不會替你挑出程度剛好的書，也不會整本自動翻譯。書籍由你選擇，原文由你閱讀；VocabReader 負責把困難的部分變得可理解，並把你選中的學習內容帶回後續複習。
