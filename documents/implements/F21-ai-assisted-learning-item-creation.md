...
資料及 Markdown 學習內容，產生可檢視、編輯、排除及恢復的
...
- **So that** 我能避免重複內容、修正 AI 草稿，並在明確確認後一次加入生詞庫
...
- 使用者可編輯標題、類型、CEFR、語義及原始 Markdown，並預覽安全渲染結果。
...
- 按下提交後，App 程式以目前編輯後的標題重新查詢候選項目，再讓 AI 只比較這些候選
  的語義，避免草稿建立後的編輯或其他批次先提交造成重複。
...
- **Scenario 7：查看、編輯、排除與恢復草稿**
  - **Given** AI 回覆下方顯示待確認批次元件
  - **When** 使用者打開中央浮層，編輯草稿並排除後再恢復其中一筆
  - **Then** 浮層安全顯示修改內容及每筆目前狀態
  - **And** 只有仍包含的最新草稿內容會參與提交

- **Scenario 8：草稿批次持久恢復**
  - **Given** 對話中有尚未提交且包含編輯／排除狀態的草稿批次
  - **When** 使用者切換對話或重新啟動 App 後返回
  - **Then** 原 AI 回覆仍顯示批次元件
  - **And** 開啟後內容與狀態和離開前一致

- **Scenario 9：提交前重查並略過新重複**
  - **Given** 草稿建立或編輯後，生詞庫已出現同標題且同語義項目
  - **When** 使用者提交批次
...
| TC13 | Edit, exclude and restore | Batch has multiple drafts | Edit one, exclude another, then restore it | Latest validated values and states persist and control submission | Critical |
...
| TC15 | Recheck after edit/concurrent submit | Draft title/sense changes or another batch wins | Submit | Candidates are re-queried; new duplicates are skipped and reported | Critical |
...
| TC20 | Production Electron flow | Built desktop App with local database | Explain annotations, create/edit/exclude/submit cards, restart | Runtime skill, persisted component, modal and resulting library items work end to end | High |
...
- 對話 store 升級為 version 2，向後讀取 version 1，保存 invitation、建立請求、
  草稿編輯／排除、垃圾桶 match 及提交結果。
- 閱讀頁與生詞庫頁加入「新增學習卡片」；解釋標記完成後顯示「加入生詞庫」。
  空目標與歧義澄清可跨重新啟動延續，下一個直接回答仍會先查候選。
- AI 訊息下方顯示批次按鈕；中央可捲動 modal 支援安全 Markdown 預覽、欄位編輯、
  排除／恢復、垃圾桶還原及明確提交。
...
| 7 編輯／排除／恢復 | Pass | Controller 與 dialog 測試 |
...
4. 為解析後邀請、批次元件、置中浮層、編輯、排除／恢復及狀態恢復建立 Renderer
   失敗測試。
5. 完成最小實作並逐層通過目標測試。
