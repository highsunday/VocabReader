...
本模組以 `en`、`ja`、`zh-TW` 三種學習語言建立互斥工作區。切換學習語言會一起切換
...
Settings 保存 `learningLanguage` 與三組 `explanationLanguages`。讀取設定時，
`explanationLanguage` 是目前工作區值的便利投影。一般 AI、標記解析、學習卡解釋與批改
原因使用講解語言；閱讀測驗題目、選項、問答答案及修正版使用學習語言。

## Existing-data migration

首次升級會先保存不可變 legacy SQLite snapshot，再由同一 snapshot 產生四個資料庫：
`en`、`ja`、`zh-TW` 及待分類 `other`。刪除非目標項目時依 foreign-key cascade 一起分流
排程與複習事件。完成 marker 使重啟可重入；若中途停止，下一次由 snapshot 重建全部
stage，不使用半完成來源。
...
外層 `vocabreader-learning-language-backup` ZIP 包含三份各自通過既有 checksum、EPUB 與
SQLite 驗證的工作區備份、共享 `settings.json`，以及存在時的待分類 SQLite。預覽同時
提供每個工作區數量與待分類數量；確認後完整取代三個工作區與共享設定，再重新啟動。
...
- `learning-language-data-backup-service.test.ts`：三工作區封裝、分區預覽、設定與完整還原。
