...
> F69 邊界：每個學習語言工作區各自擁有本模組的一份書庫、閱讀狀態、範圍與標記；
> EPUB 導入目前工作區，不依 metadata 自動跨區搬移。閱讀版面偏好仍跨工作區共用。
...
正式恢復狀態以 `readingState` 為準；載入沒有此欄位的舊索引時，會從既有欄位建立
相容預設。EPUB 重新解析後若已保存的章節 ID 不再存在，書庫會優先沿用另一個仍有效
的閱讀章節引用；兩者都失效時回到書籍總覽並把章節引用正規化為 `null`，不任意猜測
新章節。進度百分比、閱讀區段與仍可對應章節的標記不受影響。
...
- 書庫對外回傳的 `lastChapterId` 與 `readingState.chapterId` 必須指向目前章節集合；
  EPUB parser 升級留下的失效引用必須在讀取邊界安全正規化。
