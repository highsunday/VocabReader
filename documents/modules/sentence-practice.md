...
> F69 邊界：來源不再固定英文。每個工作區只抽取 `language` 等於目前學習語言且已有
> 確認複習紀錄的項目；草稿、修正版、用法與三篇範例使用學習語言，說明使用講解語言。
> 活動統計也按工作區隔離。
...
- 每次 snapshot 將今日完成數回報 `App`；完成後立即更新，並在下一個本地午夜自動重查。
- `App` 以 `max(goal - completed, 0)` 計算 badge；目標為 0 時不渲染 badge。
